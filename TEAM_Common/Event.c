...
void EVNT_Init(void) {
  uint8_t i;
  /* The critical section here is only needed if the EVNT_Init is called
   * during main code operation, therefore when Interrupts are enabled
   */
  CS1_CriticalVariable();
